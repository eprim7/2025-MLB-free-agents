...
overall_stats = ['WAR', 'Rbat+', 'OPS+', 'woba']

# COPY THE DATA AND CALL SCALER ON IT 
hitters_norm = hitters.copy()
hitters_norm[contact_stats + power_stats + overall_stats] = scaler.fit_transform(hitters[contact_stats + power_stats + overall_stats])
...
# RANK HITTERS OVERALL
hitters_norm['overall_score'] = (
    hitters_norm['WAR'] * 0.3 + 
    hitters_norm['OPS+'] * 0.3 +
    hitters_norm['Rbat+'] * 0.2 + 
    hitters_norm['woba'] * 0.2
)
...
top_overall = hitters_norm.sort_values('overall_score', ascending=False)
...
contact_stats = ['solidcontact_percent', 'batting_avg', 'on_base_percent', 'woba']
overall_stats = ['WAR', 'FIP', 'woba', 'ERA+']
...
pitchers_norm['FIP'] = 1 - pitchers_norm['FIP']
pitchers_norm['solidcontact_percent'] = 1 - pitchers_norm['solidcontact_percent']
pitchers_norm['batting_avg'] = 1 - pitchers_norm['batting_avg']
pitchers_norm['on_base_percent'] =  1 - pitchers_norm['on_base_percent']
...
pitchers_norm['contact_score'] = (
    pitchers_norm['solidcontact_percent'] * 0.3 +
    pitchers_norm['batting_avg'] * 0.3 +
    pitchers_norm['on_base_percent'] * 0.2 +
    pitchers_norm['woba'] * 0.2
)

# RANK PITCHERS OVERALL
pitchers_norm['overall_score'] = (
    pitchers_norm['WAR'] * 0.3 +
    pitchers_norm['FIP'] * 0.3 + 
    pitchers_norm['woba'] * 0.2 +
    pitchers_norm['ERA+'] * 0.2
)

top_strikeout = pitchers_norm.sort_values('strikeout_score', ascending=False)
top_contact = pitchers_norm.sort_values('contact_score', ascending=False)
